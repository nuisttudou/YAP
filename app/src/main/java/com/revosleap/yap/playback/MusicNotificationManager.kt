 ...
 * 通知
 */
class MusicNotificationManager internal constructor(private val mMusicService: MusicService) {//通知
...
//    }
    ...
    private fun notificationAction(action: String): NotificationCompat.Action {
        ...
        when (action) {
            ...
            PLAY_PAUSE_ACTION ->
                icon = if (mMusicService.mediaPlayerHolder?.getState() != PlaybackInfoListener.State.PAUSED)//更新通知中的播放图标
                    R.drawable.ic_pause
                else
                    R.drawable.ic_play
