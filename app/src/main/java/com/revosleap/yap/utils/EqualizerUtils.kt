...
//    }
    ...
    internal fun openEqualizer(activity: Activity, mediaPlayer: MediaPlayer?) {//打开均衡器
        val sessionId = mediaPlayer?.audioSessionId

        if (sessionId == AudioEffect.ERROR_BAD_VALUE) {//发生错误
            notifyNoSessionId(activity)
        } else {
            try {
                val effects = Intent(AudioEffect.ACTION_DISPLAY_AUDIO_EFFECT_CONTROL_PANEL)
                effects.putExtra(AudioEffect.EXTRA_AUDIO_SESSION, sessionId)//调用外部效果
